      ...
      <header className="flex flex-col items-center pt-8 pb-16 px-6 shrink-0 relative bg-[#3BC1A8]">
        ...
        <div className="text-center space-y-1 mb-4">
          <div className="flex items-center justify-center gap-2">
            <h1 className="text-2xl font-black text-white drop-shadow-sm">{userProfile?.username || "Guest"}</h1>
            {isVerified && (
        ...
        {hasManagementRole && (
          <section className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-2.5">
              ...
              {(userProfile?.isCoinseller || userProfile?.isAdmin) && (<button onClick={() => router.push(userProfile?.isAdmin ? '/admin/award' : '/coinseller/award')} className="w-full h-16 rounded-[1.5rem] bg-white border border-amber-100 flex items-center px-5 gap-4"><div className="w-11 h-11 rounded-xl bg-amber flex items-center justify-center"><Award className="w-5 h-5 text-amber-600" /></div><div className="flex-1 text-left"><span className="text-gray-900 font-black text-[13px] block">Award coins</span><span className="text-gray-400 text-[9px] font-bold block">Grant coins to users</span></div><ChevronRight className="w-4 h-4 text-gray-200" /></button>)}
